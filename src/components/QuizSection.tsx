  ...
  return (
    <LessonLayout>
      <SectionHeader
        ...
        action={
          <div className="flex space-x-2 glass p-1 rounded-xl shadow-sm">
            <button
      ...
      {activeTab === 'mcq' && (
        <div className="space-y-8">
          {mcqs.map((q, idx) => (
            <PracticeCard key={idx} className="p-8 hover:shadow-xl hover:shadow-indigo-500/10 transition-all duration-300 relative group glass-card">
              ...
              <div className="grid grid-cols-1 gap-3 pl-2">
                {q.options.map((opt, optIdx) => {
                  ...
                  let containerClass = "border-white/20 hover:bg-white/10 hover:border-indigo-300 glass";
                  let Icon = Circle;
                  let iconClass = "text-slate-400";

                  if (mcqSubmitted) {
                    if (isCorrectAnswer) {
                      containerClass = "bg-teal-500/10 border-teal-400 text-teal-800";
                      Icon = CheckCircle;
                      iconClass = "text-teal-600";
                    } else if (isSelected) {
                      containerClass = "bg-rose-500/10 border-rose-300 text-rose-700 animate-shake";
                      Icon = XCircle;
                      iconClass = "text-rose-500";
                    } else {
                      containerClass = "opacity-40 border-white/10 bg-white/5";
                    }
                  } else if (isSelected) {
                    containerClass = "bg-indigo-500/20 border-indigo-400 text-indigo-700 shadow-md shadow-indigo-500/10";
                    Icon = CheckCircle;
      ...
      {activeTab === 'cloze' && (
        <div className="space-y-8">
          {clozes.map((q, idx) => {
            ...
            return (
              <PracticeCard key={idx} className="p-8 hover:shadow-xl hover:shadow-indigo-500/10 transition-all duration-300 relative glass-card">
                    ...
                    <button
                      ...
                      className={`px-5 py-2.5 text-sm font-semibold rounded-xl border-2 transition-all active:scale-95
                        ${currentAnswer === opt
                          ...
                          : 'glass text-slate-700 border-white/20 hover:border-indigo-300 hover:text-indigo-600 hover:bg-white/30'
                        }
